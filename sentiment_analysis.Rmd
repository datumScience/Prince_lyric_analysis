...
#prince_tidy is in tokenized format with one row per word along with the song from which it came from 
```

```{r}
#find distinct word count per song
...
#large number of songs in the dataset donot have a release date
#each circle represents a song
```
```{r}
#song count per year
...
#the most prolific years are 1998 and 1996
#the gap is for the songs that have no release date in the dataset
```
```{r}
...
```
```{r}
#checking how many words in the lyrics are present in the lexicons
prince_tidy %>%
...
```
```{r}
#sentiment analysis with events that happened in Prince's life
