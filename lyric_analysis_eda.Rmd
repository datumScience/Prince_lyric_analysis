...
library(gridExtra) # to view plots together
...
#Read the dataset
```{r}
prince_data <- read.csv("prince_raw_data.csv",stringsAsFactors = FALSE)
names(prince_data)
```

```{r}
prince <- prince_data %>% select (lyrics = text , song, year, album, peak, us_pop = US.Pop , us_rnb = US.R.B)
glimpse(prince[139,])
```
...
```{r}
prince <- prince %>% mutate(charted = ifelse(prince$peak %in% 1:100, "Charted", "Uncharted"))
```
...
```{r}
#plotting the frequency of charted and uncharted songs by decades
prince %>% filter(decade != "NA") %>%
...
This indicates that his most active decade was 1990s.
...
```
Although more songs were released in the 1990s, most of Prince's songs were on top of the billboard in the 1980s.
...
library(kableExtra) # create a formatted HTML table
library(formattable) #color_tile function
...
```{r}
#creating a tidy text version of prince dataset
...
```{r}
prince_word_counts <- prince_words_filtered %>%
  count(word,sort=TRUE)

wordcloud2(prince_word_counts[1:300,],size=0.5)
```

```{r}
#wordcloud2(prince_word_counts[1:300, ], figPath = "guitar_icon.png", 
 #          color = "random-dark", size = 1.5)

letterCloud(prince_word_counts[1:300, ], word = "PRINCE", size = 2)
```
